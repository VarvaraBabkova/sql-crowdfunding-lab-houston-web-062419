...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_name
  ...
  where projects.id = pledges.project_id group by projects.title order by projects.title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"select users.name, users.age, sum(pledges.amount) from users, pledges
where pledges.user_id = users.id group by users.name order by users.name;"
end
...
"select users.name, sum(pledges.amount) from users, pledges where pledges.user_id = users.id
...
where projects.id = pledges.project_id and projects.category = 'music' group by projects.category;x "
end
